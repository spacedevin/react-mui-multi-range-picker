...
describe('Shared Library Functions (used by this package)', () => {
  ...
  describe('getRangesAsIndividualDates', () => {
    ...
    test('returns all dates in a single range', () => {
      const ranges: DateRange[] = [{ start: new Date('2025-01-01'), end: new Date('2025-01-03') }];
    ...
    test('handles single day range', () => {
      const ranges: DateRange[] = [{ start: new Date('2025-01-01'), end: new Date('2025-01-01') }];
  ...
  describe('isDateInRanges', () => {
    ...
    test('handles corrupt date ranges gracefully', () => {
      const badRanges: DateRange[] = [{ start: new Date('invalid'), end: new Date('2025-01-05') }];
  ...
  describe('findOverlappingRanges', () => {
    test('finds overlapping ranges', () => {
      ...
      expect(findOverlappingRanges(ranges, new Date('2025-01-03'), new Date('2025-01-12')).length).toBe(2);
    });

    test('finds single overlapping range', () => {
      const ranges: DateRange[] = [{ start: new Date('2025-01-01'), end: new Date('2025-01-05') }];
      expect(findOverlappingRanges(ranges, new Date('2025-01-03'), new Date('2025-01-04')).length).toBe(1);
    });

    test('returns empty for non-overlapping selection', () => {
      const ranges: DateRange[] = [{ start: new Date('2025-01-01'), end: new Date('2025-01-05') }];
      expect(findOverlappingRanges(ranges, new Date('2025-01-10'), new Date('2025-01-15')).length).toBe(0);
    });

    test('finds range contained within selection', () => {
      const ranges: DateRange[] = [{ start: new Date('2025-01-03'), end: new Date('2025-01-04') }];
      expect(findOverlappingRanges(ranges, new Date('2025-01-01'), new Date('2025-01-10')).length).toBe(1);
    });
  });

  describe('updateRangesWithSelection', () => {
    test('adds new range when no overlap', () => {
      expect(updateRangesWithSelection([], new Date('2025-01-01'), new Date('2025-01-05'), false).length).toBe(1);
    });

    test('removes overlapping ranges', () => {
      const ranges: DateRange[] = [{ start: new Date('2025-01-01'), end: new Date('2025-01-05') }];
      const result = updateRangesWithSelection(ranges, new Date('2025-01-03'), new Date('2025-01-04'), false);
      expect(result.length).toBeGreaterThanOrEqual(0);
    });

    test('merges ranges when mergeRanges is true', () => {
      const ranges: DateRange[] = [{ start: new Date('2025-01-01'), end: new Date('2025-01-05') }];
      const result = updateRangesWithSelection(ranges, new Date('2025-01-06'), new Date('2025-01-10'), true);
      expect(result.length).toBe(1);
      expect(result[0].start).toEqual(new Date('2025-01-01'));
      expect(result[0].end).toEqual(new Date('2025-01-10'));
    });
    ...
    test('handles reversed dates', () => {
      const ranges: DateRange[] = [];
      const result = updateRangesWithSelection(ranges, new Date('2025-01-10'), new Date('2025-01-05'), false);
      expect(result.length).toBe(1);
    });
  });
  ...
  describe('shouldUpdateDragDate', () => {
    ...
    test('returns true when dates are different days', () => {
      expect(shouldUpdateDragDate(new Date('2025-01-15'), new Date('2025-01-16'))).toBe(true);
    });

    test('returns false when new date is invalid', () => {
      expect(shouldUpdateDragDate(new Date('2025-01-15'), new Date('invalid'))).toBe(false);
    });
  });
